...
def show_resipe_preview(resipeId):
    ...
    print(resipe.name)
    return render_template(
        "ResipePreview.html", user=current_user, resipe=resipe, key=key()
    )
...
def show_addons_list():
    ...
    print(addons)
    return render_template(
        "AddonsList.html", user=current_user, addons=addons, key=key()
    )
...
def show_addon_preview(addonId):
    addon = Addon.query.get(addonId)
    return render_template(
        "AddonPreview.html", user=current_user, addon=addon, key=key()
    )
...
def add_addon():
    if request.method == "POST":
        ...
        if len(name) < 2:
            ...
            new_addon = Addon(name=name, what_is_it=what_is_it)
...
def show_nirn_preview(nirnId):
    ...
    print(resipe.name)
    return render_template(
        "NirnPreview.html", user=current_user, resipe=resipe, key=key()
    )
