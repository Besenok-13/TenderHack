...
class User(db.Model, UserMixin): #UserMixin для пихания класса User в flask
    ...
    favorite_resipes = db.relationship("Resipe", secondary=Favorite_Resipes, backref="lovers")
    favorite_nirn_resipes = db.relationship("Nirn", secondary=Favorite_Nirn, backref="lovers")
    filter_resipes = db.relationship("Resipe", secondary=Filter_Resipes, backref="filters")
...
class Resipe(db.Model):
    ...
    lover_users = db.relationship("User", secondary=Favorite_Resipes, backref="favorite_resipe")
    add_time = db.Column(db.DateTime(timezone=True), default=func.now())
    ingridients_in_resipe = db.relationship("Ingridient", secondary = Ingridients_In_Resipe, backref=db.backref("used_resipe"))
    filter_user_resipes = db.relationship("User", secondary = Filter_Resipes, backref=db.backref("filtered_resipe"))
...
class Nirn(db.Model):
    ...
    quantity = db.Column(db.Integer, default=20)
    image = db.Column(db.String(300))
    lover_users = db.relationship("User", secondary=Favorite_Nirn, backref="favorite_resipe_nirn")
    add_time = db.Column(db.DateTime(timezone=True), default=func.now())
    ingridients_in_resipe = db.relationship("NirnIngridient", secondary = NirnIngridients_In_Resipe, backref=db.backref("used_resipe_nirn"))
...
class NirnIngridient(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100))
    resipe_where_use=db.relationship("Nirn", secondary = NirnIngridients_In_Resipe, backref="used_ingr_nirn")
...
class Ingridient(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100))
    resipe_where_use=db.relationship("Resipe", secondary = Ingridients_In_Resipe, backref="used_ingr")
